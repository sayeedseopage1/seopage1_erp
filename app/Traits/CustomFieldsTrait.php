...
trait CustomFieldsTrait
{
        ...
        foreach ($group['fields'] as $field) {
            ...
            if (isset($field['value'])) {
                if (is_array($field['value'])) {
                    $insertData['value'] = json_encode($field['value']);
    ...
    public function updateCustomFieldData($fields)
    {
        foreach ($fields as $key => $value) {
            ...
            $fieldType = CustomField::find($id)->type;

            $value = ($fieldType == 'date') ? Carbon::createFromFormat(global_setting()->date_format, $value)->format('Y-m-d') : $value;
