...
use App\Models\User;
class SuggestionDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('subject', function($row) {
                $title= '';
                $title .= '<a href="'. route('suggestions.show', [$row->id]) . '" class="text-darkest-grey show-table-row openRightModal">'.ucfirst($row->subject).'</a>';
                return $title;


            })
            ->addColumn('suggestion_type', function($row) {

                return ucfirst($row->suggestion_type);
    ...
    public function query(Suggestion $model)
    {
        ...
        if ($startDate !== null && $endDate !== null) {
            $model->where(function ($q) use ($startDate, $endDate) {
                $q->whereBetween(DB::raw('DATE(suggestions.`created_at`)'), [$startDate, $endDate]);

                $q->orWhereBetween(DB::raw('DATE(suggestions.`updated_at`)'), [$startDate, $endDate]);
            });
        }
        ...
        if ($request->searchText != '') {
            $model = $model->where(function ($query) {
                $query->where('suggestions.subject', 'like', '%' . request('searchText') . '%')
                    ;
        ...
        return $this->builder()
            ...
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["suggestions-table"].buttons().container()
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.subject') => ['data' => 'subject', 'name' => 'subject', 'exportable' => false, 'title' => __('Subject')],
              __('app.suggestion_type') => ['data' => 'suggestion_type', 'name' => 'issue_type', 'exportable' => false, 'title' => __('Suggestion')],
