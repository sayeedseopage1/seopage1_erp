...
class ProductDescriptionDataTable extends DataTable
{
    /**
     ...
     */
    public function query(ProductDescription $model)
    {
        $request = $this->request();
        $productDescriptions = $request->productDescriptions;

        return $model->newQuery()
            ->whereIn('id', call_user_func_array('array_merge', $productDescriptions));
    }
    ...
    {
        return $this->builder()
            ->setTableId('product-description-table')
          ...
          //->dom($this->domHtml)
            ...
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["product-description-table"].buttons().container()
    ...
    {
        return 'ProductDescription_' . date('YmdHis');
    }
}
