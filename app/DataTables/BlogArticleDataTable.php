...
class BlogArticleDataTable extends DataTable
{
     ...
     *
     * @param \App\Models\BlogArticle $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(BlogArticle $model)
    {
        $request = $this->request();
        $blogArticles = $request->blogArticles;

        return $model->newQuery()
            ->whereIn('id', call_user_func_array('array_merge', $blogArticles));
    }
          ...
          //->dom($this->domHtml)
            ...
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["blog-article-table"].buttons().container()
