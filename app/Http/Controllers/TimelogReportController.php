    ...
    public function getTimeLog(Request $request, $type)
    {
      ...
      foreach ($data as $item) {
        ...
        if($item->project_id == null)
        {
            ...
            $item->total_minutes =ProjectTimeLog::where('user_id',$item->employee_id)->whereBetween('created_at', [$startDate, $endDate])->groupBy('created_at')->sum('total_minutes');
            $item->number_of_session =ProjectTimeLog::where('user_id',$item->employee_id)->whereBetween('created_at', [$startDate, $endDate])->groupBy('created_at')->count();
