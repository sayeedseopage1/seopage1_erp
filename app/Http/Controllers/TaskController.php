...
class TaskController extends AccountBaseController
{
    ...
    public function TaskApprove(Request $request)
    {
        ...
        {
            $text = Auth::user()->name . ' Approved the task ' . $task_status->heading ;
    ...
    public function TaskRevision(Request $request)
    {
       ...
       // dd($actions);
        ...
        {
            $revision_task = Task::where('id',$request->task_id)->first();
            $text = Auth::user()->name . ' Gave revision for task ' . $revision_task->heading;
            $link = '<a href="' . route('tasks.show', $task_status->id) . '">' . $text . '</a>';
        }else{
    ...
    public function clientApproval(Request $request)
    {
        ...
        if(!$previous_count){
            ...
            }elseif($project->deal->project_type == 'fixed' && $duration_from_submit <= 100){
