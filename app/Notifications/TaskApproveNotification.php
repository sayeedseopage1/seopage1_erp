...
class TaskApproveNotification extends Notification
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
       ...
       ;
       $header= '<p>
          <h1 style="color: red; text-align: center;" >' . __('Task Approved '.$task->task_short_code.'') .'</b>'.'
      </h1>';
      $body= '<p>
        '.'Your task '.$task->heading.' has been approved by '.$sender->name.'. To check the details, follow this link.'.
     '</p>'
     ;
     $content =
   ...
   ;
          ...
          ->subject(__('[No Reply] Task Approved') )
  
          ->greeting(__('email.hello') . ' ' . mb_ucwords($notifiable->name) . ',')
          ->markdown('mail.task.task_approved', ['url' => $url, 'greet'=> $greet,'content' => $content, 'body'=> $body,'header'=>$header, 'name' => mb_ucwords($notifiable->name)]);
    }
