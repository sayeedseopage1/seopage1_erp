    ...
    return(
        <Dropdown>
            ...
            <Dropdown.Menu>
                    ...
                    {
                        (!goal || !data || isTeamsFetching || usersIsFetching) ? (
                            <span>Loading...</span>
                        ):(
                        <ol type='1' style={{listStyle:'inside', padding: '0 20px', margin: '0'}}>                         

                        {data?.bidder !== undefined && (
                        <LI
                            text="$p got $p for converting the lead."
                            words= {[
                                `${data?.bidder}`,
                                `${tv ? '$':''}${data?.bidder_amount}`
                            ]}
                        />
                        )}

                        {data?.qualified_by !== undefined && (
                        <LI
                            text="$p got $p for qualifying the deal."
                            words= {[
                                `${data?.qualified_by}`,
                                `${tv ? '$':''}${data?.qualified_amount}`
                            ]}
                        />
                        )}

                        {data?.required_defined_by !== undefined && (
                        <LI
                            text="$p got $p for defined requirements."
                            words= {[
                                `${data?.required_defined_by}`,
                                `${tv ? '$':''}${data?.required_defined_amount}`
                            ]}
                        />
                        )} 

                        {data?.proposal_made_by !== undefined && (
                        <LI
                            text="$p got $p for sending proposal to the client."
                            words= {[
                                `${data?.proposal_made_by}`,
                                `${tv ? '$':''}${data?.proposal_made_amount}`
                            ]}
                        />
                        )}   
                        ...
                        {data?.negotiation_started_by !== undefined && (
                        <LI
                            text="$p got $p for starting negotiation with the client."
                            words= {[
                                `${data?.negotiation_started_by}`,
                                `${tv ? '$':''}${data?.negotiation_started_amount}`
                            ]}
                        />
                        )} 


                        {data?.milestone_breakdown_by !== undefined && (
                        <LI
                            text="$p got $p for sharing milestone with the client."
                            words= {[
                                `${data?.milestone_breakdown_by}`,
                                `${tv ? '$':''}${data?.milestone_breakdown_amount}`
                            ]}
                        />
                        )} 


                        {data?.deal_won_by !== undefined && (
                        <LI
                            text="$p got $p for marking this deal as won."
                            words= {[
                                `${data?.deal_won_by}`,
                                `${tv ? '$':''}${data?.won_deal_amount}`
                            ]}
                        />
                        )}  
                        </ol>
