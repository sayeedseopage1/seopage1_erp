...
const dailySubmissionApiSlice = apiSlice.injectEndpoints({
    endpoints: (build) => ({
        ...
        getAllDailySubmission: build.query({
            query: (searchParam) =>
                `/account/tasks/all-daily-submissions?${searchParam}`,
            providesTags: ["ALL_DAILY_SUBMISSION_STATUS"],
        }),
        getDailySubmissionCategories: build.query({
            query: () => `account/get-today-submission-data`,
            providesTags: ["ALL_DAILY_SUBMISSION_CATEGORIES"],
        }),
    }),
});
...
export const {
    ...
    useGetDailySubmissionCategoriesQuery,
