...
import DisputesExportButton from "./components/DisputExportButton";
...
const Disputes = () => {
                ...
                if (res) {
                    const data = _.filter(res, (d) => {
                        if (filter.status === "Pending") {
                            return (
                                d.status === 0 &&
                                _.size(d.conversations) === 0 &&
                                !d.resolved_by
                            );
                        } else if (filter.status === "In Progress") {
                            return (
                                d.status === 0 &&
                                (_.size(d.conversations) !== 0 || d.resolved_by)
                            );
    ...
    React.useEffect(() => {
        ...
        const raisedByMe = _.filter(
            disputes,
            (data) => data?.raised_by?.id === auth?.getId()
        );
        const resolveByAdmin = _.filter(
            disputes,
            (data) => data?.status && data?.authorized_by?.id === auth?.getId()
        );
        const resolveByTeamLead = _.filter(
            disputes,
            (data) =>
                data.status &&
                !data?.need_authrization &&
                data?.resolved_by?.id === auth?.getId()
        );
        ...
        let filterData = _.filter(disputes, (dispute) => {
            if (dispute && dispute.conversations) {
                let indexOf = dispute.conversations.findIndex(
                    (d) => !d.replies
                );
                if (indexOf !== -1) {
                    return true;
        ...
        const teamLeadAction = _.size(
            _.filter(disputes, (d) => !d.status && !d.need_authrization)
        );
        const needActionForAdmin = _.size(
            _.filter(disputes, (d) => !d.status && d.need_authrization)
        );
        ...
        const won = _.size(
            _.filter(
                disputes,
                (f) => f.status && f?.winner?.id === auth.getId()
            )
        );
        const lost = _.size(
            _.filter(
    ...
    return (
        <React.Fragment>
            ...
            <div className="py-2 py-md-4 px-2 px-md-5 w-full">
                ...
                >
                    <DisputesExportButton filter={filters} />
