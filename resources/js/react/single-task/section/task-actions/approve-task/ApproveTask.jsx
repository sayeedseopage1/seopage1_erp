...
import TaskRating from "../../../../global/TaskRating";

const ApproveTask = ({ task, status, auth }) => {
    const dispatch = useDispatch();
    const [showApproveForm, setShowApproveForm] = useState(false);
    const [completedWithInDeadline, setCompletedWithInDeadline] =
        useState(null);
    const [submittedStar, setSubmittedStar] = useState(null);
    const [fullfilledStar, setFullfilledStar] = useState(null);
    const [comment, setComment] = useState("");

    const [oldSubmittion, setOldSubmittion] = useState([]);
    const [latestSubmittion, setLatestSubmittion] = useState({});

    const [approveSubmittedTask, { isLoading }] =
        useApproveSubmittedTaskMutation();
    const { data: getSubmittedTask, isFetching } = useGetSubmittedTaskQuery(
        task?.id
    );
    ...
    const isValid = () => {
        ...
        if (!completedWithInDeadline) {
            error.completedWithInDeadline =
                "Please rate the task completion within deadline!";
            count++;
        }
        if (!submittedStar) {
            error.submittedStar =
                "Please rate how beautifully the task is submitted!";
            count++;
        }
        if (!fullfilledStar) {
            error.fullfilledStar =
                "Please rate how perfectly the task requirements are fulfilled!";
            count++;
        }
                                        ...
                                        <div className="card card-body">
                                            {_.size(oldSubmittion) > 0 ? (
                                                _.map(oldSubmittion, (task) => (
                                                    <SubmittedWorkCard
                                                        key={`${task.id}_${task?.submission_no}`}
                                                        data={task}
                                                        isLoading={isFetching}
                                                    />
                                                ))
                                            ) : (
                                                <span>No Old Submittion!</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="form-group">
                                        ...
                                        <div className="">
                                            <TaskRating
                                                ...
                                                onChange={
                                                    setCompletedWithInDeadline
                                                }
                                                readonly={false}
                                            />
                                            {err.completedWithInDeadline && (
                                                <p
...
"
                                    ...
                                    <div className="form-group">
                                        <label
                                            ...
                                            className="font-weight-bold"
                                        >
                                            How Perfectly The Task Requirements
                                            Are Fullfilled?
                                            <sup className="f-16">*</sup>
                                        </label>
                                        <div className="">
...
const SubmittedWorkCard = ({
    ...
    if (isLoading) {
        return (
            <div
                className={`sp1_st--approve-card mb-3 ${className}`}
                style={style}
            >
    ...
    return (
        <div className={`sp1_st--approve-card mb-3 ${className}`} style={style}>
            <div className="sp1_st--approve-card-header">
                <span>
                    {latest ? "Latest" : "Old"} Submittion{" "}
                    {!latest && `(${data?.submission_no})`}
                </span>
                <span>
                ...
                <div className="mb-2">
                    ...
                    <div
                        ...
                        dangerouslySetInnerHTML={{
                            __html:
                                data?.text ??
                                `<span style="color:rgb(180,188,196);">Comment Not Available</span>`,
                        }}
                    />
                </div>
