...
const ColorItem = ({color, desc}) => { 
    ...
    return (
        <div className='position-relative px-2'>
            <div onClick={() => handleCopy(color)} className='py-1 px-3 d-inline-block mr-2 text-white rounded' style={{background: color}}>
                 {!show ? color : state.value ? <span> Copied {state.value}</span> : null}</div>
