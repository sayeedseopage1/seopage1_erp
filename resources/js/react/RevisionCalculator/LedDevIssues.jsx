...
const LeadDevIssue = () => {
    ...
    const [
        getRevisionCalculatorDataLeadIssues,
        {isFetching}
  ...
  useEffect(() => { 
    ( async () => {
        ...
        try{
            let res = await getRevisionCalculatorDataLeadIssues(`/${pm_id}?${queryString}`).unwrap();
            const arr = [];
            _.forEach(res.lead_dev__issues, d => {
                arr.push({
                    ...d,
