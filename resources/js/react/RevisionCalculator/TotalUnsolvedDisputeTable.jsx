...
import {  useLazyGetRevisionCalculatorDataTotalUnsolveDisputeQuery } from '../services/api/revisionCalculatorApiSlice';


import { TotalUnsolvedDisputeTableColumns } from './TotalUnsolvedDisputeTableColumns';



// const data = projectElaborationData(50);

const TotalUnsolvedDisputeTable = () => {
    ...
    const [
        getRevisionCalculatorDataTotalUnsolveDispute,
        {isFetching}
  ...
  useEffect(() => { 
    ( async () => {
        ...
        try{
            let res = await getRevisionCalculatorDataTotalUnsolveDispute(`/${pm_id}?${queryString}`).unwrap();
            const arr = [];
            _.forEach(res.dispute_not_resolved, d => {
                arr.push({
                    ...d,
          ...
          <div className="sp1_modal-content-wrapper">
              <div className={`sp1_modal-panel ${styles.modal_panel}`}>
                  ...
                  <div className={`sp1_modal-head ${styles.modal_title_bar}`}>
                      <div className="sp1_modal-title pl-2">Total Unsolved Dispute Table</div>
                      <Button 
                        onClick={goBack} 
                  ...
                  <div className={`sp1_modal-body ${styles.modal_body}`}>
                      ...
                      <DataTable
                          data={data} 
                          margeRow={true}
                          tableName='RevisionTotalUnsolvedDisputeTableColumns'
                          columns={TotalUnsolvedDisputeTableColumns}
                          pageIndex={pageIndex}
                          perPageRow={nRows}
...
export default TotalUnsolvedDisputeTable 