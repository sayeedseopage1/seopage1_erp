...
const columns = [
            ...
            return (
                <div style={{ minWidth: "150px" }}>
                    ...
                    {(row?.transition_hours || row?.transition_minutes) && (
                        <div>
                            {row?.transition_hours} Hours{" "}
                            {row?.transition_minutes} Minutes
                        </div>
                    )}
                </div>
            );
    ...
    },
    {
        ...
        cell: (row) => (
            <span>
                {row?.forgot_to_track_task_id ? (
                    <a href={`/account/tasks/${row?.forgot_to_track_task_id}`}>
                        {row?.forget_task_heading}
                    </a>
                ) : (
    ...
    },
    {
        ...
        cell: (row) => (
            <span>
                {row?.related_to_any_project ? (
                    <a
                        href={`/account/projects/${row?.related_to_any_project}`}
                    >
                        {row?.project_name}
                    </a>
                ) : (
    ...
    },
    {
        ...
        sortAccessor: "",
        cell: (row) => (
            <div>{row?.reason_for_less_tracked_hours_a_day_task}</div>
        ),
    },
    {
    ...
    },
    {
        ...
        sorted: false,
        sortAccessor: "",
        cell: (row) => <ReportResolvePreviewModal row={row} />,
    },
    {
        ...
        sorted: false,
        cell: () => {
            return <quote>No commnet yet!</quote>;
        },
    },
];
...
const TimeLogHIstoryModalTable = ({ row, filter, tableName }) => {
    ...
    const [getTimeLogHistoryDetails, { isFetching }] =
    ...
    useEffect(() => {
        getTimeLogHistoryDetails(
            `${row?.employee_id}?start_date=${filter?.start_date}&end_date=${filter?.end_date}`
        )
            .unwrap()
            .then((res) => {
                            ...
                            {!isFetching &&
                                ...
                                _.map(renderData, (row) => {
                                    let data = {
                                        ...
                                        user:
                                            usersObject &&
                                            new User(usersObject[row?.user_id]),
                                    };
                                    return (
