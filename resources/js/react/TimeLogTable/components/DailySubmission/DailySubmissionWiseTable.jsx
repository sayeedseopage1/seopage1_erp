                ...
                value?.map((item, index) => {
                    ...
                    const className =
                        value.length === index + 1
                            ...
                            : "sp1_tlr_td f-14";
                    rows.push(
                        <tr className="sp1_tlr_ds_tr" key={item?.unique_id}>
                                ...
                                if (col.group) {
                                    ...
                                    return (
                                        <React.Fragment key={col?.id}>
                                            {col.cell({
                                                row: item,
                                                className: `${className} sp1_drag_col_${col?.id}`,
                                            })}
                                        </React.Fragment>
                                    );
                                }
            ...
            <div className="p-3">
                ...
                >
                    ...
                    r_table"
                        draggable
                    >
                        <thead className="sp1_daily_sub_thead">
                            <tr className="sp1_tlr_ds_tr">
                                {_.map(_columns, (column, index) => {
                                    // console.log({column});
                                    return (
                                        <TableDragAbleHeader
                                            key={column?.id}
                                            className="sp1_daily_sub_th"
                                            column={column}
                                            columns={_columns}
                                            onSort={() => {}}
