...
const EditProjectManagerGuideline = ({
    isOpen,
    close,
    data,
    openTaskForm,
    ...
    const [photoshopReferenceURL, setPhotoshopReferenceURL] =
        React.useState("");
    const [designRefURL, setDesignRefURL] = React.useState([
        { id: "lgqsz", url: "" },
    ]);
    ...
    const [updateProjectGuideline, { isLoading }] =
    ...
    const themeDetailsAuthorized = () => {
        const d = _.find(
            data?.pm_task_guideline_authorization,
            (d) =>
                _.replace(_.lowerCase(d?.name), /\s+/g, "_") === "theme_details"
        );
    ...
    const designDetailsAuthorized = () => {
        const d = _.find(
            data?.pm_task_guideline_authorization,
            (d) =>
                _.replace(_.lowerCase(d?.name), /\s+/g, "_") ===
                "design_details"
        );
    ...
    const colorSchemaAuthorized = () => {
        const d = _.find(
            data?.pm_task_guideline_authorization,
            (d) =>
                _.replace(_.lowerCase(d?.name), /\s+/g, "_") === "color_schema"
        );
                ...
                if (designFileType === "" || designFileType === "--") {
                    ...
                    if (designFileType === "XD/Figma") {
                        ...
                        if (photoshopReferenceURL === "") {
                            err.photoshop =
                                "You Have to Provide Reference Link!";
                            count++;
                        ...
                        _.map(designRefURL, (item) => {
                            if (item.url === "") {
                                ...
                                err.designRef =
                                    "Provided Value Must Be an URL!";
                                count++;
                            }
                        });

                        if (designRefDescription === "") {
                            err.designRefDescripton =
                                "You have to Provide Instruction!";
                            count++;
                        }
                    }
                }
        ...
        if (!pluginResearchAuthorized() && edit.plugin_research) {
            ...
            if (plugin === "yes") {
                ...
                if (pluginGoogleDrive === "") {
                    err.pluginGoogleDrive =
                        "You Have to Provide Plugin Google Drive URL";
                    count++;
                ...
                if (pluginDescription === "") {
                    err.pluginDescription =
                        "You Have to Provide Instruction of This Plugin!";
                    count++;
                }
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!designDetailsAuthorized() && edit.design_details) {
            formData = {
                ...
                reference_link: _.compact(
                    _.map(designRefURL, (item) => item.url)
                ),
        ...
        if (!colorSchemaAuthorized() && edit.color_schema) {
            formData = {
                ...
                color_description: _.compact(
                    _.map(secondaryColors, (item) => item.description)
                ),
            };
        }
                    ...
                    <div className="sp1_modal-body sp1_task_create_modal_body">
                        ...
                        <Switch>
                            ...
                            <div className="py-4 px-3">
                                ...
                                <Switch.Case
                                    condition={
                                        data?.task_category?.toLowerCase() !==
                                        "design"
                                    }
                                >
                                    <React.Fragment>
...
EditProjectManagerGuideline.propTypes = {
