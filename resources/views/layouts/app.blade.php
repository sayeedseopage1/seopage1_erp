...
<body id="body" class="{{ user()->dark_theme ? 'dark-theme' : '' }} {{ user()->rtl ? 'rtl' : '' }}">
    ...
    <div class="body-wrapper clearfix">
        ...
        <section class="main-container bg-additional-grey" id="fullscreen">
            ...
            <?php
            $deal_id= App\Models\Deal::where('status','pending')->orderBy('award_time','asc')->where('pm_id',Auth::id())->first();
            if ($deal_id != null) {
    ...
    <?php
    $deal_id= App\Models\Deal::where('status','pending')->orderBy('award_time','asc')->first();
    if ($deal_id != null) {
      $to = \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', \Carbon\Carbon::now());
