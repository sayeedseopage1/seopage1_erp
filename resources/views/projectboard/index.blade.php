...
@section('filter-section')
    ...
    <x-filters.filter-box>
        ...
        <x-filters.more-filter-box>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('modules.projects.assignTo')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" id="assignedTo" data-live-search="true" data-container="body" data-size="8">
                            ...
                            @if ($viewUnassignedTasksPermission == 'all')
                                <option value="unassigned">@lang('modules.projects.unassigned')</option>
                            @endif
                        </select>
                    </div>
                </div>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('modules.projects.assignBy')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
        ...
        <div class="d-lg-flex d-md-flex d-block my-3">
            ...
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if ($addTaskPermission == 'all' || $addTaskPermission == 'added')
                    <x-forms.link-primary :link="route('projects.create')" class="mr-lg-3 mr-1 openRightModal float-left" icon="plus" data-redirect-url="{{ url()->full() }}">
                        @lang('app.add')
                        @lang('app.project')
            ...
            {{--   @if (user()->permission('add_status') == 'all')
                    <x-forms.button-secondary icon="plus" class="mr-lg-3 mr-1 float-left" id="add-column">
                        @lang('modules.projects.addBoardColumn')
                    </x-forms.button-secondary>
                @endif
                --}}
            ...
            <div class="btn-group" role="group">
                ...
                <a href="{{ route('projectboards.index') }}" class="btn btn-secondary f-14 btn-active" data-toggle="tooltip"
                    data-original-title="@lang('modules.projects.projectBoard')"><i class="side-icon bi bi-kanban"></i></a>
...
@push('scripts')
    ...
    <script>
        ...
        $('body').on('click', '.load-more-projects', function() {
            ...
            var totalTasks = $(this).data('total-projects');
            ...
            var url = "{{ route('taskboards.load_more') }}?startDate=" + encodeURIComponent(startDate) +
                ...
                '&assignedTo=' +
                assignedTo + '&projectID=' + projectID + '&category_id=' + categoryId + '&label_id=' + labelId +
                '&searchText=' + searchText + '&columnId=' + columnId +
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    ...
                    if (response.load_more == 'show') {
                        $('#drag-container-' + columnId).closest('.b-p-body').find('.load-more-projects');
        ...
        //     const url = "{{ route('projectboards.create') }}";
        ...
        //     var url = "{{ route('projectboards.edit', ':id') }}";
