...
<div id="hours_spent_in_revision{{ count($spent_revision_developer_lead_data) }}" class="modal fade" tabindex="-1"
            ...
            <div class="modal-header">
                <div class="modal-title">
                    <h4>Total logged hours for all submitted task in this period:
                        {{ round($spent_revision_developer_lead_data->sum('total_submissions_log_time_in_min') / 60, 2) }}</h4>
                    <h4>Total logged hours in tasks with revisions:
                        {{ round($spent_revision_developer_lead_data->sum('total_log_time_in_min') / 60, 2) }}</h4>
                    <h4>Logged hours in revisions: {{ round($spent_revision_developer_lead, 2) }}</h4>
                    ...
                    <tbody>
                        @foreach ($spent_revision_developer_lead_data as $row)
                            <tr>
                                ...
                                <td>
                                    {{ round($row?->firstTaskSubmission?->timeLogs?->sum('total_minutes') / 60, 2) . 'h' ?? 0 }}
                                </td>
                                <td>
                                    {{ round($row?->revisions?->sum('total_log_time') / 60, 2) . 'h' ?? 0 }}
                                </td>
                            </tr>
                        @endforeach
...
@foreach ($spent_revision_developer_data_lead as $key => $row)
