                             ...
                             ?>
                              <tr>
                                <td>
                                  ...
                                  <td class="pl-20 text-capitalize "><a class="text-darkest-grey openRightModal RightModal" id="RightModal" title="{{$project->project_name}}" href="/account/projects/{{$project->id}}" target="_blank">{{Str::limit($project->project_name,15)}}</a></td>
                                  <td class="pl-20 text-capitalize">{{$deal->actual_amount}}{{$deal->original_currency->currency_symbol}}</td>
                                  <td class="pl-20 text-capitalize"><a class="openRightModal"  href="/account/clients/{{$project->client_id}}" title="{{$project->client_name->user_name}}" data-tooltip-bottom="{{$project->client_name->user_name}}">
                                    @if($project->client_name->image != null)
                                    <img class="mr-2 taskEmployeeImg rounded-circle" data-original-title="{{$project->client_name->user_name}}" src="{{asset('user-uploads/avatar/'. $project->client_name->image)}}"

                                    alt="{{$project->client_name->user_name}}">
                                    @else
                                    <img class="mr-2 taskEmployeeImg rounded-circle" src="{{asset('user-uploads/avatar/avatar_blank.png')}}"

                                    title="{{$project->client_name->user_name}}" alt="{{$project->client_name->user_name}}" >


                                    @endif
                                      </a></td>
                                  <td class="pl-20 text-capitalize">{{$project->deadline->format('Y-m-d')}}</td>
                                 ...
                                 ?>
                                  <tr>
                                    <td>
                                      ...
                                      <td class="pl-20 text-capitalize "><a class="text-darkest-grey openRightModal" title="{{$task->heading}}" href="/account/tasks/{{$task->id}}" target="_blank">{{$task->heading}}</a></td>
                                      <td class="pl-20 text-capitalize"><a class="text-darkest-grey openRightModal RightModal" title="{{$task->project->project_name}}" id="RightModal" href="/account/projects/{{$task->project->id}}" target="_blank">{{Str::limit($task->project->project_name,15)}}</a</td>
                                      <td class="pl-20 text-capitalize"><a class="openRightModal"  href="/account/clients/{{$task->project->client_id}}" title="{{$task->project->client_name->user_name}}" data-tooltip-bottom="{{$task->project->client_name->user_name}}">
                                        @if($task->project->client_name->image != null)
                                        <img class="mr-2 taskEmployeeImg rounded-circle" data-original-title="{{$task->project->client_name->user_name}}" src="{{asset('user-uploads/avatar/'. $task->project->client_name->image)}}"

                                        alt="{{$task->project->client_name->user_name}}">
                                        @else
                                        <img class="mr-2 taskEmployeeImg rounded-circle" src="{{asset('user-uploads/avatar/avatar_blank.png')}}"

                                        title="{{$task->project->client_name->user_name}}" alt="{{$task->project->client_name->user_name}}" >


                                        @endif
                                          </a></td>
                                      <td class="pl-20 text-capitalize"><a class=""  href="/account/employees/{{$user_name->name}}" title="{{$user_name->name}}" data-tooltip-bottom="{{$user_name->name}}">
                                        @if($task->project->client_name->image != null)
                                        <img class="mr-2 taskEmployeeImg rounded-circle" data-original-title="{{$user_name->name}}" src="{{asset('user-uploads/avatar/'. $user_name->image)}}"

                                        alt="{{$user_name->name}}">
                                        @else
                                        <img class="mr-2 taskEmployeeImg rounded-circle" src="{{asset('user-uploads/avatar/avatar_blank.png')}}"

                                        title="{{$user_name->name}}" alt="{{$user_name->name}}" >
