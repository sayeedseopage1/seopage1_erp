...
<div class="modal fade"
    id="avg_task_approval_client{{ count($average_number_of_attempts_needed_for_approval_by_client_data) }}"
                    ...
                    <tbody>
                        @foreach ($average_number_of_attempts_needed_for_approval_by_client_data as $row)
                            <tr>
                                ...
                                <td
                                    title="@foreach (array_key_exists($row->id, $average_number_of_attempts_needed_for_approval_by_client_array) ? $average_number_of_attempts_needed_for_approval_by_client_array[$row->id] : [] as $history)({{ ++$loop->index }}): {{ $history }}&#xA; @endforeach">
                                    @if (array_key_exists($row->id, $average_number_of_attempts_needed_for_approval_by_client_array))
                                        ({{ count($average_number_of_attempts_needed_for_approval_by_client_array[$row->id]) }})
                                    @else
                                        (1)
                                    @endif
                                </td>
