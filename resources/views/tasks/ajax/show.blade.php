    ...
    <div class="row">
                ...
                <div class="card-header bg-white border-bottom-grey text-capitalize justify-content-between p-20">
                    <div class="row">
                        <div class="col-lg-8 col-10">
                            ...
                            ($changeStatusPermission == 'both' && (in_array(user()->id, $taskUsers) || $task->added_by == user()->id)) || ($task->project && $task->project->project_admin == user()->id) )
                            <?php
                            $extension_request= App\Models\TaskTimeExtension::where('task_id',$task->id)->first(); ?> @if($extension_request != null) @if ($extension_request->status == 'pending')
                            ...
                            @include('tasks.modals.extensionrequest') @endif @endif
                            <?php
                              $task_user= App\Models\TaskSubmission::orderBy('id','desc')->where('task_id',$task->id)->first(); // dd($task_user->user_id,Auth::user()->id); ?> @if ($task->task_status == 'submitted' )
                            <!-- <x-forms.button-primary icon="check" data-status="completed"
                                        class="change-task-status mr-2 mb-2 mb-lg-0 mb-md-0">
                                        @lang('modules.tasks.markComplete')
                                    </x-forms.button-primary> -->
                            ...
                            @include('tasks.modals.taskapprove') @include('tasks.modals.taskrevision') @endif @endif @endif @if($task->board_column_id != 6) @if ($task->boardColumn->slug != 'completed' && !is_null($task->is_task_user)) @if
                            (is_null($task->userActiveTimer))
                            ...
                            @else
                            <x-forms.button-secondary id="resume-timer-btn" icon="play-circle" data-time-id="{{ $task->userActiveTimer->activeBreak->id }}">@lang('modules.timeLogs.resumeTimer')</x-forms.button-secondary>
                            @endif @endif @endif @if(Auth::user()->role_id == 5 || Auth::user()->role_id == 6) @if($task->status != "completed")
                            ...
                            <?php
                                              $extension=App\Models\TaskTimeExtension::orderBy('id','desc')->where('task_id',$task->id)->where('user_id',Auth::id())->first(); ?> @if($task->status != "completed") @if($extension == null)
                            ...
                            @endif @endif @include('tasks.modals.markcomplete') @include('tasks.modals.timeextension')

                            <?php


                                  $extension_status=App\Models\TaskTimeExtension::orderBy('id','desc')->where('task_id',$task->id)->first(); ?> @if($extension_status != null) @if($extension_status->status == 'pending' || $extension->status
                            != 'approved')
                        ...
                        <div class="col-lg-4 col-2 text-right">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                    ...
                                    @endif @if ($editTaskPermission == 'all' || ($editTaskPermission == 'added' && $task->added_by == user()->id) || ($task->project && $task->project->project_admin == user()->id))
                                    <a class="dropdown-item openRightModal" href="{{ route('tasks.edit', $task->id) }}">@lang('app.edit') @lang('app.task')</a>

                                    <hr class="my-1" />
                                    @endif @php $pin = $task->pinned() @endphp @if ($pin)
                                    <a class="dropdown-item" href="javascript:;" id="pinnedItem" data-pinned="pinned">@lang('app.unpin') @lang('app.task')</a>
                                    @else
                                    <a class="dropdown-item" href="javascript:;" id="pinnedItem" data-pinned="unpinned">@lang('app.pin') @lang('app.task')</a>
                                    @endif @if (($taskSettings->copy_task_link == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                ...
                <div class="card-body">
                    ...
                    <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                        ...
                        <p class="mb-0 text-dark-grey f-14 w-70">
                            @if ($task->project_id) @if ($task->project->status == 'in progress')
                            ...
                            @endif
                            <a href="{{ route('projects.show', $task->project_id) }}" class="text-dark-grey"> {{ $task->project->project_name }}</a>
                            @else -- @endif
                        </p>
                    </div>
                    ...
                    <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                        ...
                        @if (count($task->users) > 0) @if (count($task->users) > 1) @foreach ($task->users as $item)
                        ...
                        @endforeach @else @foreach ($task->users as $item)
                    ...
                    <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                        ...
                        <p class="mb-0 text-dark-grey f-14 w-70">
                            ...
                            <span class="badge badge-secondary" style="background-color: {{ $label->label_color }}">{{ $label->label_name }} </span>
                    ...
                    @endif --}} @if (in_array('gitlab', user_modules()) && isset($gitlabIssue))
                    <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                        <p class="mb-0 text-lightest f-14 w-30 d-inline-block text-capitalize">
                            GitLab
                        </p>
                    ...
                    @endif @if (($taskSettings->task_category == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                    <x-cards.data-row :label="__('modules.tasks.taskCategory')" :value="$task->category->category_name ?? '--'" html="true" />
                    @endif @if (($taskSettings->description == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                    <x-cards.data-row :label="__('app.description')" :value="!empty($task->description) ? $task->description : '--'" html="true" />
                    @endif
                    <?php

                    $task_name=App\Models\Task::where('id',$task->dependent_task_id)->first(); ?> @if($task_name != null) @if (($taskSettings->description == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) ||
                    ...
                    @endif @endif {{-- Custom fields data --}} @if (($taskSettings->custom_fields == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles())) @if (isset($fields))
                    ...
                    <x-cards.data-row
                        :label="$field->label"
                        :value="(!is_null($task->custom_fields_data['field_' . $field->id]) && $task->custom_fields_data['field_' . $field->id] != '' ? \Carbon\Carbon::parse($task->custom_fields_data['field_' . $field->id])->format(global_setting()->date_format) : '--')"
                    />
                    @endif @endforeach @endif @endif
                </div>
            </div>

            @if ((($taskSettings->files == 'yes' || $taskSettings->sub_task == 'yes' || $taskSettings->comments == 'yes'|| $taskSettings->time_logs == 'yes'|| $taskSettings->notes == 'yes' || $taskSettings->history == 'yes') &&
            in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
            <!-- TASK TABS START -->
            <div class="bg-additional-grey rounded my-3">
                <div class="s-b-inner s-b-notifications bg-white b-shadow-4 rounded">
                    <x-tab-section class="task-tabs">
                        ...
                        <x-tab-item class="ajax-tab" :active="(request('view') === 'file' || !request('view'))" :link="route('tasks.show', $task->id).'?view=file'">@lang('app.file')</x-tab-item>
                        @endif @if (($taskSettings->sub_task == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                        <x-tab-item class="ajax-tab" :active="(request('view') === 'sub_task')" :link="route('tasks.show', $task->id).'?view=sub_task'"> @lang('modules.tasks.subTask')</x-tab-item>
                        @endif @if (($taskSettings->comments == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles())) @if ($viewTaskCommentPermission != 'none')
                        <x-tab-item class="ajax-tab" :active="(request('view') === 'comments')" :link="route('tasks.show', $task->id).'?view=comments'"> @lang('modules.tasks.comment')</x-tab-item>
                        @endif @endif @if (($taskSettings->time_logs == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                        <x-tab-item class="ajax-tab" :active="(request('view') === 'time_logs')" :link="route('tasks.show', $task->id).'?view=time_logs'">
                            @lang('app.menu.timeLogs') @if ($task->active_timer_all_count > 0)
                            <i class="fa fa-clock text-primary f-12 ml-1"></i>
                            @endif
                        </x-tab-item>
                        @endif @if (($taskSettings->notes == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles())) @if ($viewTaskNotePermission != 'none')
                        <x-tab-item class="ajax-tab" :active="(request('view') === 'notes')" :link="route('tasks.show', $task->id).'?view=notes'">@lang('app.notes')</x-tab-item>
                        @endif @endif @if (($taskSettings->history == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                        <x-tab-item class="ajax-tab" :active="(request('view') === 'history')" :link="route('tasks.show', $task->id).'?view=history'">@lang('modules.tasks.history') </x-tab-item>
                        @endif @if (($taskSettings->history == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                        <x-tab-item class="ajax-tab" :active="(request('view') === 'deliverables')" :link="route('tasks.show', $task->id).'?view=deliverables'">@lang('Deliverables') </x-tab-item>
                        @endif
                    </x-tab-section>
        ...
        <div class="col-sm-3">
            <x-cards.data>
                ...
                <p class="f-w-500"><i class="fa fa-circle mr-1 text-yellow" style="color: {{ $task->boardColumn->label_color }}"></i>{{ $task->boardColumn->column_name }}</p>
                ...
                <div class="col-12 px-0 pb-3 d-flex">
                    @if ($task->is_private)
                    <span class="badge badge-secondary"><i class="fa fa-lock"></i> @lang('app.private')</span>&nbsp; @endif @if ($pin)
                    <span class="badge badge-success"><i class="fa fa-thumbtack"></i> @lang('app.pinned')</span>
                    @endif
                </div>
                @endif @endif @if (($taskSettings->start_date == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                <div class="col-12 px-0 pb-3 d-lg-flex d-block">
                    ...
                    <p class="mb-0 text-dark-grey w-50 f-14">
                        @if(!is_null($task->start_date)) {{ $task->start_date->format(global_setting()->date_format) }} @else -- @endif
                    </p>
                </div>
                @endif @if (($taskSettings->due_date == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                <div class="col-12 px-0 pb-3 d-lg-flex d-block">
                    <p class="mb-0 text-lightest w-50 f-14 text-capitalize">{{ __('app.dueDate') }}</p>
                    <p class="mb-0 text-dark-grey w-50 f-14">
                        @if(!is_null($task->due_date)) {{ $task->due_date->format(global_setting()->date_format) }} @else -- @endif
                    </p>
                </div>
                @endif @if($task->original_due_date != null) @if (($taskSettings->due_date == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                <div class="col-12 px-0 pb-3 d-lg-flex d-block">
                    <p class="mb-0 text-lightest w-50 f-14 text-capitalize">{{ __('Original Due Date') }}</p>
                    <p class="mb-0 text-dark-grey w-50 f-14">
                        @if(!is_null($task->original_due_date)) {{ \Carbon\Carbon::parse($task->original_due_date)->format('d-m-Y') }} @else -- @endif
                    </p>
                </div>
                @endif @endif @if (($taskSettings->time_estimate == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles())) @if ($task->estimate_hours > 0 ||
                ...
                @endif @endif @php $totalMinutes = $task->timeLogged->sum('total_minutes') - $breakMinutes; $timeLog = intdiv($totalMinutes, 60) . ' ' . __('app.hrs') . ' '; if ($totalMinutes % 60 > 0) { $timeLog .= $totalMinutes % 60 . ' '
                . __('app.mins'); } @endphp @if (($taskSettings->hours_logged == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                <div class="col-12 px-0 pb-3 d-lg-flex d-block">
                    <p class="mb-0 text-lightest w-50 f-14 text-capitalize">
