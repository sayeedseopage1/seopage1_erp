...
@push('scripts')
 <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.19.2/jquery.validate.min.js"></script>
    ...
    <script>
        $('#dm-leads-table').on('preXhr.dt', function(e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["dm-leads-table"].draw();
        }
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
              ...
              //console.log('id');
                if (result.isConfirmed) {
                    var url = "{{ route('digital-marketing-lead.destroy', ':id') }}";
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#dm-leads-table input:checkbox:checked").map(function() {
                return $(this).val();
                //console.log= ('rowIds');
            }).get();
        ...
        $('#dm-leads-table').on('change', '.change-status', function() {
        ...
        function changeStatus(leadID, statusID) {
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (response.status == "success") {
                        $.easyBlockUI('#dm-leads-table');
                        $.easyUnblockUI('#dm-leads-table');
                        showTable();
                        resetActionButtons();
                        deSelectAll();
                    }
